//! Public scaffolding for the fantome generator: types, WAD prep, the
//! hashtable streamer, and the orchestrator.
//!
//! The proprietary build step itself (`build_fantome`) lives in
//! `skin0_swap_impl.rs`, which is gitignored and only compiled in when the
//! `private-impl` Cargo feature is enabled. Open-source builds get a stub that
//! returns an error.

// Items below are only consumed when the private-impl feature is enabled;
// in the OSS stub build the orchestrator returns immediately.
